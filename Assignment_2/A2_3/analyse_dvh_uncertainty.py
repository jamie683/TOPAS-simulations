"""
A2_3 — plot ensemble DVHs with ±1σ shaded bands for the 1-field photon plan.

Reads ``A2_3/output/uncertainty/aggregated.npz`` (produced by
``run_uncertainty.py``) and produces an uncertainty-aware version of the
existing ``dvh_photon_1field.png``. Each structure appears as a single mean
curve with a ±1σ band of the same colour.

Usage:
    python3 A2_3/analyse_dvh_uncertainty.py
"""
...
OUTPUT_PATH = os.path.join(SCRIPT_DIR, "output", "dvh_photon_1field_uncertainty.png")

# Match the colour conventions of analyse_dvh.py for visual consistency.
STRUCTURE_COLORS = {
    "tumour": "red",
    "ptv":    "magenta",
    "lung_r": "orange",
...
def main():
    if not os.path.isfile(AGG_PATH):
        ...
        print("Run A2_3/run_uncertainty.py first.")
        return
    ...
    title = "Cumulative DVHs — Single 1 MeV Photon Field"
    if n_replicates:
    ...
    # Summary table with per-replicate D-value statistics.
    print()
