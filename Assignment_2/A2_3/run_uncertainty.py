"""
A2_3 — Monte Carlo uncertainty driver for the single 1 MeV photon field.

Runs ``dose_scoring.txt`` ``N_REPLICATES`` times with independent RNG seeds,
computes full cumulative DVHs per structure directly from the TOPAS voxel-level
dose CSVs (using the RTStruct masks that ``A2_4/sweep_beams.py`` already knows
how to build), and aggregates the replicates into mean ± 1σ bands.

Outputs (under ``A2_3/output/uncertainty/``):
...
from disk. Pass ``--force`` to force every replicate to be re-run.

Usage:
    python3 A2_3/run_uncertainty.py
    python3 A2_3/run_uncertainty.py --force
"""
...
TEMPLATE_PATH = os.path.join(SCRIPT_DIR, "dose_scoring.txt")
UNCERT_DIR    = os.path.join(SCRIPT_DIR, "output", "uncertainty")
CT_DIR        = os.path.join(PROJECT_ROOT, "CTData")

# Mask keys we want DVHs for. Matches the naming in sweep_beams.build_structure_masks.
...
def replicate_basename_rel(k):
    """Basename relative to the project root, no extension (TOPAS appends .csv)."""
    return os.path.join("A2_3", "output", "uncertainty", f"replicate_{k:02d}")
...
def main():
    ...
    print("=" * 78)
    print("A2_3 — PHOTON SINGLE-FIELD MONTE CARLO UNCERTAINTY")
    print("=" * 78)
    ...
    for k, seed in enumerate(SEEDS):
        ...
        # Use cached NPZ if available and neither --force nor --rescore.
        ...
        # --rescore: recompute DVH from existing CSV, skip TOPAS.
    ...
    for key in STRUCTURE_KEYS:
        ...
        print(f"{display.get(key, key):<14s} {fmt('D95'):>14s} "
              f"{fmt('D50'):>14s} {fmt('D02'):>14s} {fmt('Dmax'):>14s}")

    print("\nDone.")
    print("Next: python3 A2_3/analyse_dvh_uncertainty.py")
