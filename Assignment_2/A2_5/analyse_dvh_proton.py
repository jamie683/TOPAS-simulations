"""
Section 2.5: Cumulative DVH plotting from a single VICTORIA proton export.

Usage:
    cd A2_5 && python3 analyse_dvh_proton.py
    or from project root:
    python3 A2_5/analyse_dvh_proton.py

Expects one DVH CSV file exported from VICTORIA in output/.
Default filename: dvh_proton.csv (change DVH_FILE below if needed).

Expected format (tab or comma delimited):
    dose    GTVp    Lung_R    Heart    SpinalCord    ...
    0       1       1         1        1

The script auto-detects delimiter, dose column, and structure columns.
Dose axis is normalised to % of maximum dose for plotting.

Outputs:
    output/dvh_proton.png               cumulative DVH plot
...
def print_summary(dose_pct, structures):
    print("\n" + "=" * 68)
    print("PROTON DVH SUMMARY (dose as % of maximum)")
    print("=" * 68)
...
def plot_cumulative_dvh(dose_pct, structures, output_path):
    ...
    ax.set_title("Cumulative DVHs \u2014 Optimised Single Proton Field (Normalised Dose)", fontsize=14)
    ax.set_xlim(0, 105)
    ax.set_ylim(0, 105)
...
def main():
    ...
    output_path = os.path.join(OUTPUT_DIR, "dvh_proton.png")
